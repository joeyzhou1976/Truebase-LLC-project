...
const outFile = path.join(root, 'unbranded_products_payload.js');
...
const categories = [
  {
    ...
    files: ['DSC00362', 'DSC00363', 'DSC00364', 'DSC00365', 'DSC00366', 'DSC00367'],
    hero: 'DSC00362',
    alt: 'Yellow small pet habitat and bath box with ventilated lid',
  ...
  },
  {
    ...
    eyebrow: 'Hydration',
    files: ['DSC00372', 'DSC00373', 'DSC00377'],
    hero: 'DSC00372',
    alt: 'Blue and white automatic pet water fountain top view',
  ...
  },
  {
    ...
    files: ['DSC00379', 'DSC00380', 'DSC00381', 'DSC00382', 'DSC00383'],
    hero: 'DSC00379',
    alt: 'Gray slow feeder enrichment insert for pets',
  ...
  },
  {
    ...
    files: ['DSC00385', 'DSC00386', 'DSC00387', 'DSC00389'],
    hero: 'DSC00385',
    alt: 'Owl-shaped laser cat toy side view with rounded rotating head design',
  ...
  },
  {
    ...
    files: ['DSC00391', 'DSC00392', 'DSC00393'],
    hero: 'DSC00393',
    alt: 'Green multi-function pet feeding station with bowls and water bottle holder',
  ...
  },
  {
    ...
    eyebrow: 'Pet Care',
    files: ['DSC00395', 'DSC00396', 'DSC00397'],
    hero: 'DSC00397',
    alt: 'Green pet cleaning and litter basin with scoop and raised floor texture',
...
const usedStems = [...new Set(categories.flatMap(c => c.files))];
const imageMap = {};
for (const stem of usedStems) {
  const file = `${stem}.jpg`;
  const category = categories.find(c => c.files.includes(stem));
  imageMap[stem] = {
    stem,
    file,
...
const payload = `(() => {
  ...
  function uploadImage(item) {
    const existing = request("GET", base + "/wp-json/wp/v2/media?search=" + encodeURIComponent(item.stem) + "&per_page=20");
    const found = existing.json && existing.json.find(m => (m.title && m.title.rendered || "").includes(item.stem));
    if (found) {
      request("POST", base + "/wp-json/wp/v2/media/" + found.id, JSON.stringify({
        title: item.stem,
        alt_text: item.alt,
        caption: item.alt,
        description: item.alt
      }), { "Content-Type": "application/json" });
      media[item.stem] = { id: found.id, url: found.source_url, alt: item.alt };
      return;
    }
    ...
    const res = request("POST", base + "/wp-json/wp/v2/media", bytes, {
      ...
      "Content-Disposition": 'attachment; filename="pet-product-' + item.stem.toLowerCase() + '.jpg"'
    });
    if (!res.ok) {
  ...
  function alt(stem) { return images[stem] && images[stem].alt || "Pet product image"; }
  function gallery(category) {
    return category.files.map(stem => '<figure><img class="tb-editable-img" src="' + img(stem) + '" alt="' + alt(stem) + '" loading="lazy"><figcaption>Detail View</figcaption></figure>').join("");
  }
  ...
  const categorySections = categories.map(cat =>
    '<article id="' + cat.key + '" class="tb-category">' +
      '<div><div class="tb-product-img"><img class="tb-editable-img" src="' + img(cat.hero) + '" alt="' + cat.alt + '" loading="lazy"></div></div>' +
      '<div class="tb-category-copy"><span class="tb-label">' + cat.eyebrow + '</span><h3>' + cat.title + '</h3><p>' + cat.desc + '</p>' + featureList(cat.features) + '<div class="tb-gallery">' + gallery(cat) + '</div></div>' +
    '</article>'
  ).join("");

  const cards = categories.map(cat =>
    '<a class="tb-card" href="#' + cat.key + '" style="text-decoration:none;color:inherit"><span class="tb-label-pill">' + cat.eyebrow + '</span><h3>' + cat.title + '</h3><p>' + cat.desc + '</p></a>'
  ).join("");

  const schema = '<script type="application/ld+json">' + JSON.stringify({
    "@context": "https://schema.org",
    "@type": "CollectionPage",
    name: "Pet Supplies and Toys",
    description: "Pet supplies and toys including small pet habitats, automatic water fountains, slow feeders, feeding stations, laser cat toys, and pet care basins.",
  ...
  const content = css + '<div class="tb-page">' + header +
    '<main>' +
      '<section class="tb-hero"><div class="tb-wrap tb-hero-grid"><div><span class="tb-label">Products</span><h1>Pet Supplies & Toys</h1><p>Explore practical, retail-ready pet products grouped by use case: hydration, feeding, enrichment, cleaning, and small-pet care. Each image is uploaded to the WordPress media library, so photos can be replaced or optimized later in Elementor.</p><div class="tb-actions"><a class="tb-btn" href="' + base + '/truebase-request-catalog/">Request Catalog</a><a class="tb-btn tb-btn-outline" href="' + base + '/truebase-request-samples-quote/">Get Samples / Quote</a></div></div><div class="tb-media hero"><img class="tb-editable-img" src="' + img('DSC00393') + '" alt="Multi-function pet feeding station product hero image" loading="eager"></div></div></section>' +
  ...
  const update = request("POST", base + "/wp-json/wp/v2/pages/" + productsPageId, JSON.stringify({
    ...
    content,
    excerpt: "Browse pet supplies and toys, including small pet habitats, automatic water fountains, slow feeders, laser cat toys, feeding stations, and pet cleaning basins.",
    meta: {
      _yoast_wpseo_title: "Pet Supplies & Toys | TrueBase",
      _yoast_wpseo_metadesc: "Browse pet supplies and toys, including small pet habitats, automatic water fountains, slow feeders, laser cat toys, feeding stations, and cleaning basins.",
      _yoast_wpseo_focuskw: "pet supplies and toys"
    }
  }), { "Content-Type": "application/json" });
