...
const outFile = path.join(root, 'truebase_memorial_products_payload.js');

const base = 'https://truebaseholding.com';
const productsPageId = 86;

const images = {
  hero: 'https://images.pexels.com/photos/4947386/pexels-photo-4947386.jpeg?auto=compress&cs=tinysrgb&w=1400',
  urns: 'https://images.pexels.com/photos/36195990/pexels-photo-36195990.jpeg?auto=compress&cs=tinysrgb&w=1200',
  ...
  craft: 'https://images.pexels.com/photos/34586365/pexels-photo-34586365.jpeg?auto=compress&cs=tinysrgb&w=1400'
};
...
const productSections = products.map(product => (
  `<article id="${product.key}" class="tb-product">` +
    `<div class="tb-product-media"><img src="${images[product.key]}" alt="TrueBase ${esc(product.title)}" loading="lazy"><span class="tb-mark">TrueBase</span></div>` +
...
const content = css + `<div class="tb-page">${header}<main>` +
  `<section class="tb-hero"><div class="tb-wrap tb-hero-grid"><div><span class="tb-label">Products</span><h1>Premium Pet Memorial Product Catalog</h1><p>Explore high-end memorial product lines selected around popular buyer demand: urns, keepsake boxes, ashes jewelry, photo frames, glass art, engraved sets, garden markers, and sympathy gifts. Each line supports TrueBase branding, partner packaging, and customization for professional aftercare programs.</p><div class="tb-actions"><a class="tb-btn" href="${base}/truebase-request-catalog/">Request Catalog</a><a class="tb-btn tb-btn-outline" href="${base}/truebase-request-samples-quote/">Get Samples / Quote</a></div></div><div class="tb-media"><img src="${images.hero}" alt="TrueBase premium memorial product retail display" loading="eager"><span class="tb-mark">TrueBase</span></div></div></section>` +
...
const payload = `(() => {
  ...
  function getNonce() {
    ...
    const patterns = [
      /wpApiSettings[^<]+nonce["']?\\s*[:=]\\s*["']([a-f0-9]+)["']/i,
      /"nonce":"([a-f0-9]+)"/i,
      /rest_nonce["']?\\s*[:=]\\s*["']([a-f0-9]+)["']/i
    ];
    for (const pattern of patterns) {
      const match = html.match(pattern);
      if (match) return match[1];
    }
    throw new Error("Could not find WordPress REST nonce. Admin status " + admin.status + ", length " + html.length);
  }
  ...
  const update = request("POST", base + "/wp-json/wp/v2/pages/" + productsPageId, body, {
