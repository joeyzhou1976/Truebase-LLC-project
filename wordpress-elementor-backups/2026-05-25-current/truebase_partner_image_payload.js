(() => {
  ...
  const pages = request('GET', '/wp-json/wp/v2/pages?slug=truebase-become-a-partner&context=edit', undefined, {
    'X-WP-Nonce': nonce
  });
  const page = pages.json && pages.json[0];
  if (!page) {
    return JSON.stringify({ ok: false, error: 'Page not found for slug truebase-become-a-partner' });
  }

  let content = page.content && page.content.raw ? page.content.raw : page.content.rendered;
  const doc = new DOMParser().parseFromString(content, 'text/html');
  const candidates = [
    doc.querySelector('.tb-hero img'),
    doc.querySelector('.tb-media img'),
  ...
  const update = request('POST', `/wp-json/wp/v2/pages/${page.id}`, JSON.stringify({
    content,
    excerpt: 'Partner with TrueBase through collaborative memorial product programs, sample planning, and private-label support.'
  }), {
