...
const outFile = path.join(root, 'truebase_about_page_payload.js');
const base = 'https://truebaseholding.com';

const mediaRegistry = {
  'about.hero': {
    type: 'image',
    src: 'https://truebaseholding.com/wp-content/uploads/2026/05/truebase-life-corgi-portrait.jpg',
    alt: "Life, Joey's corgi, looking at the camera",
    label: 'Life the corgi'
  },
  'about.origin': {
    type: 'image',
    src: 'https://truebaseholding.com/wp-content/uploads/2026/05/truebase-life-corgi-garden.jpg',
    alt: "Life, Joey's corgi, resting in the garden",
    label: 'Life'
  },
  'about.feedback': {
    type: 'image',
    src: 'https://images.pexels.com/photos/3184465/pexels-photo-3184465.jpeg?auto=compress&cs=tinysrgb&w=1400',
    alt: 'Professional partners discussing a TrueBase memorial product program',
...
function mediaSlot(key, className = 'tb-media', loading = 'lazy') {
...
const company = {
  name: 'Truebase Holding LLC',
  type: 'Wyoming Limited Liability Company',
  filed: 'Apr 14, 2026',
  originalId: '2026-001950978',
...
const payload = `(() => {
  ...
  const mediaRegistry = ${JSON.stringify(mediaRegistry)};
  const company = ${JSON.stringify(company)};
  const css = ${JSON.stringify(css)};
  const esc = ${esc.toString()};
  const nav = ${nav.toString()};
  const header = ${header.toString()};
  const footer = ${footer.toString()};
  const mediaSlot = ${mediaSlot.toString()};
  const mediaRegistryMarkup = ${mediaRegistryMarkup.toString()};
  const pageContent = ${aboutContent.toString()};
